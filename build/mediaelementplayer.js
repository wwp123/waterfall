				...
				for(; i<lines.length; i++) {
					...
					if (this.pattern_identifier.exec(lines[i])){
						...
						if (timecode && i<lines.length){
							...
							text = $.trim(text).replace(/(\b(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/ig, "<a href='$1' target='_self'>$1</a>");
		...
		dfxp: {
			parse: function(trackText) {
				...
				for(i = 0; i<lines.length; i++) {
					...
					text = $.trim(lines.eq(i).html()).replace(/(\b(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/ig, "<a href='$1' target='_self'>$1</a>");
					entries.text.push(text);
